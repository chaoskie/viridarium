...
2. **Scaffold the change (`SPEC-002`):** create `specs/changes/<change-name>/` with `proposal.md`, `design.md`, and `tasks.md` (full track), plus an empty `screenshots/` directory when the story touches UI (`FE-012`). Use the worklog template for step 3. Hand-rolled change folders (outside this command) must scaffold the same set - DoR item 12 checks it.
