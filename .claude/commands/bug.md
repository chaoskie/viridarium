...
1. **Intake:** if no report exists yet, file one from `templates/bug-report.md` (`bugs/BUG-NNN-...`). **Triage:** delegate to the triage subagent for reproduce path, affected context/layer, suspected cause, severity suggestion (`REV-003` scale). No fixing yet.
